...
% P_OCT_LABEL labels an octant of a 27-surrounding for later use in
% determining the Euler characteristic of the central point, and whether it
% can be removed or not.

% If the octant in question is 1
if( octant==1 )
    
    % set points in this octant to current label
    % and recursive labeling of adjacent octants
    
    % For point 1 in the 27-neighborhood: check whether it is filled in 
    idx_1 = find(cube(1,:) == 1); 
    if(~isempty(idx_1)) % if it was empty
        cube(1,idx_1) = label(idx_1); % Replace the filled in voxel with "2" instead of 1.
    end
    
    % Do the same if the point was 2
    idx_2 = find(cube(2,:) == 1); % idem: voor punt 2
    if(~isempty(idx_2))
        cube(2,idx_2) = label(idx_2);
        
        % This point is also found in octant number 2. Therefore, perform
        % this subroutine for octant number 2 too.
