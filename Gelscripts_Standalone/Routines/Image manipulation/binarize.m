...
if strcmp(binThres,'auto') == 1
    ...
    if strcmp(switch_outputlabeling,'yes') == 1
        ...
        legend('Average high','Average low','Threshold Otsu''s method','Location','northwest')
        savefig([pathstr,'/Output/',fname,'/BinInt.fig']);
        hold off
    end
else
    ...
    if strcmp(switch_outputlabeling,'yes') == 1
        ...
        xlabel('slice (#)')
        ylabel('Normalized intensity')
        legend('Average high','Average low','Set threshold','Threshold Otsu''s method','Location','northwest')
        savefig([pathstr,'/Output/',fname,'/BinInt.fig']);
        hold off
    end
end
...
if strcmp(switch_optimizationtools,'yes') == 1
    ...
    axis([-inf inf 0 1])
    savefig([pathstr,'/Output/',fname,'/BinSurf.fig']);
    hold off
end
